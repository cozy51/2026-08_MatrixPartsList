...
import { inferPlMode } from './plModes';
...
const aliases:Record<keyof Part,string[]>={balloon:['風船'],partNo:['品番'],version:['Ver.','Ver'],quantity:['数量'],name:['品名'],material:['材質・メーカー'],additionalInfo:['追加情報','備考'],unavailable:['使用不可'],unitMass:['単品質量（kg）','単品質量(kg)'],specification:['部品仕様']};
export function parsePartsList(text:string,fileName:string):PartsList{if(!text.trim())throw new Error('空のファイルです。');const rows=parseCsv(text);let plNo='',plName='';for(const row of rows)row.forEach((v,i)=>{if(v==='PLNO')plNo=row[i+1]||'';if(v==='PL名称')plName=row[i+1]||'';});const hi=rows.findIndex(r=>r.includes('品番')&&r.includes('品名')&&r.includes('風船'));if(hi<0)throw new Error('明細ヘッダー（風船・品番・品名）が見つかりません。');const header=rows[hi];const ix=(k:keyof Part)=>{for(const a of aliases[k]){const n=header.indexOf(a);if(n>=0)return n;}return-1;};const get=(r:string[],k:keyof Part)=>{const n=ix(k);return n<0?'':r[n]||'';};const parts:Part[]=[];for(const r of rows.slice(hi+1)){const balloon=get(r,'balloon'),name=get(r,'name'),partNo=get(r,'partNo');if(!balloon&&!name&&!partNo)continue;if(!name&&!partNo)continue;parts.push({balloon,partNo:partNo||'+',version:get(r,'version')||'-',quantity:get(r,'quantity'),name,material:get(r,'material'),additionalInfo:get(r,'additionalInfo'),unavailable:get(r,'unavailable'),unitMass:get(r,'unitMass'),specification:get(r,'specification')});}if(!parts.length)throw new Error('有効な明細がありません。');return{id:crypto.randomUUID(),fileName,plNo:plNo||fileName.replace(/\.(csv|xlsx?|xlsm)$/i,''),plName,plVersion:extractPlVersion(fileName),modeId:inferPlMode(fileName,plName),parts,visible:true,importedAt:new Date().toISOString()};}
