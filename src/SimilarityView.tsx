...
type Props = { lists: PartsList[]; baseId: string; onBaseChange: (id: string) => void };

export default function SimilarityView({ lists, baseId, onBaseChange }: Props) {
  ...
  return <section className="similarity-view">
    <div className="similarity-controls">
      ...
      <label><span>基準PL</span><select value={effectiveBaseId} onChange={event => onBaseChange(event.target.value)}>{lists.map(list => <option key={list.id} value={list.id}>{plLabel(list)}</option>)}</select></label>
    </div>
    <div className="similarity-list">{results.map((result, index) => <article className={`similarity-row ${result.list.id === effectiveBaseId ? 'is-base' : ''}`} key={result.list.id}>
      ...
      <div className="similarity-name"><b>{plLabel(result.list)}</b>{isStandardPl(result.list.plNo) && <span className="standard-badge">STD</span>}<small>{result.list.plName}</small></div>
